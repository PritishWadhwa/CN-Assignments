...
void routingAlgo(vector<RoutingNode *> nd)
{
    ...
    do
    {
        ...
        for (int k = 0; k < num; k++)
        {
            // The following loop is to get the routing table of each node and compare it with the previous one
            // If there is a change in the routing table, then the flag is set to true
            routingTable.push_back(nd[k]->getTable());
        }
        for (int k = 0; k < num; k++)
        {
            // The following loop is to get the routing table of each node and compare it with the previous one
